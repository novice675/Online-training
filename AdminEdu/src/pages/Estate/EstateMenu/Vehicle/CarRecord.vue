  ...
  <div class="vehicle-record-container">
      ...
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="search-inputs">
          ...
          <el-form-item label="进出时间：">
            <el-date-picker v-model="searchForm.exitTime" type="date" placeholder="请选择进出时间" format="YYYY-MM-DD"
    ...
    <div class="table-area">
      <el-table :data="tableData" border stripe style="width: 100%" v-loading="loading"
        ...
        <el-table-column prop="vehicleType" label="车辆类型" align="center" width="100">
          <template #default="{ row }">
        ...
        <el-table-column prop="direction" label="进出方向" align="center" width="90">
          <template #default="{ row }">
            <span :class="row.direction === '出场' ? 'out-direction' : 'in-direction'">
              {{ row.direction || '出场' }}
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="exitTime" label="进出时间" align="center" width="160">
          <template #default="{ row }">
            <span>{{ formatDateTime(row.exitTime || row.startTime) }}</span>
          </template>
        </el-table-column>
...
const fetchData = async () => {
  ...
  try {
    const params = {
      ...
      exitTime: searchForm.exitTime
    }

    // 使用与CarInfo.vue相同的接口地址
...
const handleExport = async () => {
  ...
  try {
    ...
    link.download = `车辆进出记录_${new Date().toLocaleDateString('zh-CN').replace(/\//g, '-')}.png`
